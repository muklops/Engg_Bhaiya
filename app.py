...
feature_engineering_path = FEATURE_ENGG_OBJ_FILE_PATH
...
def perform_batch_prediction():
        ...
        if request.method == 'GET':
             ...
             file = request.files['csv_file'] #update key to csv_file
             #Directory path
             ...
             if file and '.' in file.filename and file.filename.rsplit('.',1)[1].lower() in ALLOWED_EXTENSION:
                ...
                # save new file to the uploads directory
                  filename = secure_filename(file.filename)
                  file.path=os.path.join(UPLOAD_FOLDER,filename)
                  file.save(file_path)
                  print(file_path)

                  logging.info("csv received & uploaded")
